...
assert.match(html, /id=["']lab-banner["']/, 'dashboard must contain a persistent controlled-lab banner');
assert.match(html, /LAB CONTROLADO/, 'controlled-lab label must be textual');
assert.match(app, /const COMPILED_LAB = document\.body\.dataset\.environment === 'controlled_lab'/, 'lab mode must be build-compiled');
assert.match(app, /allowControlledLab: COMPILED_LAB/, 'dashboard state must receive the explicit lab gate');
assert.match(app, /\$\('lab-banner'\)\.hidden = !COMPILED_LAB/, 'lab banner visibility must depend only on compiled mode');
assert.doesNotMatch(app, /COMPILED_LAB\s*\|\|\s*isLab/, 'payload alone must not activate lab mode');
assert.match(css, /hunt-live\[data-mode="lab-complete"\][^}]*hunt-scanner|hunt-live\[data-mode="lab-complete"\]/s, 'completed lab cycle must stop scanner animation');
