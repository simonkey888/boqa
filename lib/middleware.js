...
// Protects sensitive decision/autonomy/economic/policy/allocation routes.
// Key is read from BOQA_API_KEY env var. If not set, defaults to 'BOQA123'
// so the server is protected out-of-the-box without manual env configuration.
// For production, override with a stronger key via `BOQA_API_KEY=<strong-key>`.

const DEFAULT_API_KEY = 'BOQA123';
const API_KEY = process.env.BOQA_API_KEY || DEFAULT_API_KEY;
let apiKeyWarningLogged = false;

function requireApiKey(req, res, next) {
  if (!API_KEY) {
    if (!apiKeyWarningLogged) {
      console.warn('[Server] BOQA_API_KEY not set — protected routes are OPEN. Set this in production!');
