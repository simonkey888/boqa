...
function requireStrongProxyAuth(req, res, next) {
  req.boqaAuth = req.boqaAuth || {};
  setNoStore(res);
  const expectedKey = process.env.BOQA_API_KEY || '';
  const secret = process.env.BOQA_HMAC_SECRET || '';
  if (!expectedKey || !secret) {
    req.boqaAuth.apiKey = false;
    req.boqaAuth.hmac = false;
    return res.status(503).json({ error: 'private_proxy_auth_unavailable' });
  }

  verifyHmac(req, res, (hmacError) => {
    if (hmacError) return next(hmacError);
    const providedKey = req.headers['x-api-key'];
    if (!providedKey || !safeEqualText(providedKey, expectedKey)) {
      req.boqaAuth.apiKey = false;
