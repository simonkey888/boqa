...
class LocalLabRuntime {
  ...
  async scenario(origin, marker, manifest, kind) {
    ...
    return {
      result: kind === 'candidate'
        ? (reflected ? 'LAB_FINDING_CONFIRMED' : 'INDETERMINATE')
        : (reflected ? 'INDETERMINATE' : 'LAB_CONTROL_CLEAN'),
    };
  }
  ...
  async runOnce(options = {}) {
    ...
    try {
      ...
      const marker = `boqa-${crypto.createHash('sha256').update(runId).digest('hex').slice(0, 18)}`;
      const candidate = await this.scenario(candidateOrigin, marker, manifest, 'candidate');
      const control = await this.scenario(controlOrigin, marker, manifest, 'control');
