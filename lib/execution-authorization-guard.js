...
function auditTaskDecision(task, result, options = {}) {
  try {
    options.telemetry?.recordSecurityDecision('execution_guard', result, {
      phase: options.phase || 'unspecified',
      action: task?.action || 'unknown',
      target_id: task?.target_id || 'unknown',
...
async function validateTaskAsync(task, registry, options = {}) {
  ...
  if (!sync.allowed) return auditTaskDecision(task, sync, options);
    ...
    if (!result.allowed) {
      const code = item.path.startsWith('params.request_sequence[')
        ...
        : item.path.startsWith('params.steps[')
          ? 'WORKFLOW_STEP_INVALID'
          : 'TASK_URL_INVALID';
      return auditTaskDecision(task, deny(code, `${item.path}: ${result.reason}`, { cause: result.code, path: item.path }), options);
    }
  }
  return auditTaskDecision(task, allow({ urls: sync.urls }), options);
}
