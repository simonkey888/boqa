...
const { AuditTelemetry } = require('./audit-telemetry');
...
function initialize(CONFIG, OUTPUT_DIR) {
  ...
  const auditTelemetry = new AuditTelemetry();
  ...
  const verificationFarm = new VerificationFarm({
    knowledgeBase,
    agent: null,
  ...
  const runtimeMonitor = new RuntimeMonitor({ auditTelemetry });
  ...
  verificationFarm.setAgent(agent);
