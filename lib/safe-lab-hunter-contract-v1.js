...
function generateSafeLabHunterContract(options = {}) {
  ...
  const unavailableMs = Number.isFinite(options.freshness?.unavailableMs)
    ? options.freshness.unavailableMs
  ...
  const contract = {
    ...
    unavailable_after: new Date(asserted.gateCompletedMs + unavailableMs).toISOString(),
    finding_count: 1,
