...
class BrowserEgressGuard {
  ...
  async install(context) {
    ...
    await context.route('**/*', async route => {
      ...
      try { this.onDecision({ resource_type: resourceType, ...result }); } catch (_) {}
      safeRecordSecurityDecision(this.telemetry, 'browser_egress', result, {
    ...
    await context.routeWebSocket('**/*', async webSocketRoute => {
      safeRecordSecurityDecision(this.telemetry, 'browser_websocket', {
        allowed: false,
